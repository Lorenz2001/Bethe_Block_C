...
bool DataSet::Generate() {
  ...
  for (int i = 0; i < _charges.size(); i++) {
    ...
    for (int j = 0; j < _n_events[i]; j++) {
      ...
      std::cout<<i<<": tof: "<< _tof[_tof.size()-1] << " de: " << *(_dE.end()-1) <<"\n";
    }
  }
...
double DataSet::_ToF_Generator(Range<double> tof, double sigma, TRandom3& rand) {
  TRandom3 rand3;
...
double DataSet::_dE_Generator(double tof, int charge, double sigma, TRandom3& rand) {
